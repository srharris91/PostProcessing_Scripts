...
def plot_data(sim,exp,direction):
    ...
    if direction=='u':
        ...
        axavg_colorbar = axavg.contourf(-sim.Yall,sim.Xall,sim.Uavg,colormap,cmap='jet')
        plt.colorbar(axavg_colorbar,ax=axavg,ticks=[0,0.25,0.5,0.75,1.,1.25],)
        ...
        colormap=np.linspace(0.,0.6,300)
        axexp1_colorbar = axexp1.tricontourf(-sim.Yall[sim.exp_range],sim.Xall[sim.exp_range],sim.Uavg[sim.exp_range],colormap,cmap='jet')
        plt.colorbar(axexp1_colorbar,ax=axexp1,ticks=[0.,0.3,0.6])
        axexp2_colorbar = axexp2.contourf(-exp.x,exp.y,exp.v,colormap,cmap='jet',aspect='equal')
        plt.colorbar(axexp2_colorbar,ax=axexp2,ticks=[0.,0.3,0.6])
        # get rid of white lines in pdf
        for c in axavg_colorbar.collections + axexp1_colorbar.collections + axexp2_colorbar.collections:
            c.set_edgecolor("face")
    elif direction=='v':
        #colormap=np.linspace(np.nanmin(sim.Urms),np.nanmax(sim.Urms),300)
        colormap=np.linspace(0,np.nanmax(sim.Urms),300)
        axavg_colorbar = axavg.contourf(-sim.Yall,sim.Xall,sim.Urms,colormap,cmap='jet')
        plt.colorbar(axavg_colorbar,ax=axavg,ticks=[0.,0.1,0.2])
        axexp1 = plt.subplot2grid((4,140),(0,40),rowspan=2,colspan=60)
        axexp2 = plt.subplot2grid((4,140),(2,40),rowspan=2,colspan=60,sharex=axexp1,sharey=axexp1)
        axexp3 = plt.subplot2grid((4,140),(0,100),rowspan=4,colspan=40,sharey=axavg)
        ...
        #colormap=np.linspace(0.,0.6,300)
        axexp1_colorbar = axexp1.tricontourf(-sim.Yall[sim.exp_range],sim.Xall[sim.exp_range],sim.Urms[sim.exp_range],colormap,cmap='jet')
        plt.colorbar(axexp1_colorbar,ax=axexp2,ticks=[0,0.05,0.1])
        axexp2_colorbar = axexp2.contourf(-exp.x,exp.y,exp.rmsv,colormap,cmap='jet',aspect='equal')
        plt.colorbar(axexp2_colorbar,ax=axexp1,ticks=[0,0.05,0.1])
        # get rid of white lines in pdf
        for c in axavg_colorbar.collections + axexp1_colorbar.collections + axexp2_colorbar.collections:
            c.set_edgecolor("face")
    # patch to axavg
    ...
    if direction=='u':
        axexp3.xaxis.set_major_locator(plt.MultipleLocator(0.2))
    elif direction=='v':
        axexp3.xaxis.set_major_locator(plt.MultipleLocator(0.04))
    fig.tight_layout()
    # arrow path for exp
    ...
    plt.savefig(sim.save_directory+'/ave__'+direction+'.pdf',bbox_inches='tight')
